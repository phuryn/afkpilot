...
| `npm run gate` | `typecheck`, `npm test`, then touch, legacy, browser, downloads, and screenshot E2E suites. | Mandatory local relay release gate before promotion. Its `e2e:browser` leg is the only place the authenticated flow runs, so CI cannot replace it. |
| `npm run gate:ci` | `gate` without `e2e:browser`. | What `.github/workflows/ci.yml` runs on pushes and pull requests. `e2e:browser` is excluded because it needs real Clerk and Supabase credentials; every other suite blanks its credentials and is hermetic. |
...
For a relay-only change, run `npm test` plus the focused E2E command for the affected boundary. Before a change is eligible for relay promotion, run the full `npm run gate` and record the result for review.
