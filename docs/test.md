...
For a relay-only change, run `npm test` plus the focused E2E command for the affected boundary. Before a change is eligible for relay promotion, run the full `npm run gate` and record the result for review. The gate requires the sibling `grok-build-vscode` checkout for `e2e:lifecycle`; a missing host fails. The standalone `npm run e2e:lifecycle` command may still skip when the sibling is absent. There is no lifecycle CI job: record both repository commit IDs and any working-tree changes with the local result.
