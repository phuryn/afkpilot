...
| `CLOUD_POOL_SIZE` | How many cloud environments to keep built and parked, ready to hand out. Unset or `0` means no pool: every first open builds on demand, which took 25 minutes end to end (apt 58s, clone 77s, `npm ci` 20m, compile 4.6m). A parked machine bills cold storage only — $0.02/GB-month against ~2.3 GB of overlay, so 50 machines is roughly $2.30/month — while removing that wait from the user's path entirely. Requires `RELAY_PUBLIC_URL`; without it the pool refuses to start rather than building machines that can never report in. | Unset (no pool). | `50` in production. Off in dev unless you mean to buy machines. |
| `RELAY_PUBLIC_URL` | This relay's own public `https://` origin, e.g. `https://afkpilot.com`. A pooled machine reaches the relay over the internet rather than over the socket a host uses, so it has to be told where to fetch its install script, report readiness, and collect its device token. A wrong value produces machines that build perfectly and never link. | Unset. | The deployment's own public URL. |
