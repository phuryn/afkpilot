...
| `SPRITES_URL_TEMPLATE` | Shape of a sprite public URL, with `{name}` where the sprite name goes. A sprite hostname carries a per-org suffix and cannot be derived from the name, so the default is right for some orgs and wrong for others. | `https://{name}.sprites.app/` | Deployment config. |
